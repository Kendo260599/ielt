...
const FireIcon = () => ( <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5 text-orange-500"> <path fillRule="evenodd" d="M10 2c-1.716 0-3.408.106-5.07.31C3.806 2.45 3 3.414 3 4.517V5.25a.75.75 0 0 0 1.5 0V4.517c0-.28.164-.533.41-.631A20.74 20.74 0 0 1 10 3.5c1.64 0 3.26.095 4.839.277.247.1.411.35.411.631V5.25a.75.75 0 0 0 1.5 0V4.517c0-1.103-.806-2.068-1.93-2.207A21.91 21.91 0 0 0 10 2Z" clipRule="evenodd" /> <path d="M5.53 5.603a.75.75 0 0 0-1.06 1.06l2.25 2.25a.75.75 0 0 0 1.06 0l2.25-2.25a.75.75 0 0 0-1.06-1.06L9 6.84V6.75a2.25 2.25 0 0 1 2.25-2.25c.34 0 .672.083.969.233a.75.75 0 0 0 .8-.433a3.75 3.75 0 0 0-1.769-.383C8.7 4.25 7.25 5.642 7.25 7.25v.01l-1.72-1.657Z" /> <path d="M9 8.25a.75.75 0 0 0-1.5 0v5.69l-1.72-1.657a.75.75 0 0 0-1.06 1.06l2.25 2.25a.75.75 0 0 0 1.06 0l2.25-2.25a.75.75 0 0 0-1.06-1.06L9 13.19V8.25Z" /> <path d="M11.25 5.25a2.25 2.25 0 0 0-2.25 2.25v.01l1.72-1.657a.75.75 0 0 0 1.06 1.06l-2.25 2.25a.75.75 0 0 0 0 1.06l2.25 2.25a.75.75 0 0 0 1.06-1.06l-1.72-1.657V7.5a2.25 2.25 0 0 0-2.25-2.25Z" /> </svg>);
const StarIcon = () => ( <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5 text-yellow-400"> <path fillRule="evenodd" d="M10.868 2.884c.321-.662 1.215-.662 1.536 0l1.822 3.758 4.144.604c.729.106 1.018.996.49 1.505l-2.997 2.92.707 4.127c.124.725-.636 1.282-1.28.944l-3.706-1.948-3.706 1.948c-.644.338-1.405-.219-1.28-.944l.707-4.127-2.997-2.92c-.528-.509-.239-1.399.49-1.505l4.144-.604 1.822-3.758Z" clipRule="evenodd" /> </svg>);
...
const Header: React.FC<HeaderProps> = ({ user, isGuest, streak, xp, onViewProgress, onHomeClick, showHomeButton, onSignOut, onRequestSignIn }) => {
  ...
  return (
    <header className="bg-surface/80 backdrop-blur-lg border-b border-border sticky top-0 z-50">
      <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 md:px-8 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
            {showHomeButton ? (
                ...
                <>
                    <AppLogo />
                    <h1 className="hidden sm:block text-xl md:text-2xl font-bold text-primary tracking-tight">
                    IELTS Scholar
                    </h1>
                </>
            )}
        ...
        <div className="flex items-center space-x-2 sm:space-x-4">
            {(user || isGuest) && (
              <>
                <div className="hidden sm:flex items-center space-x-2 bg-orange-100 dark:bg-orange-500/20 px-3 py-1.5 rounded-full">
                    <FireIcon />
                    <span className="font-bold text-sm text-orange-600 dark:text-orange-300">{streak}</span>
                </div>
                <div className="hidden sm:flex items-center space-x-2 bg-yellow-100 dark:bg-yellow-500/20 px-3 py-1.5 rounded-full">
                    <StarIcon />
                    <span className="font-bold text-sm text-yellow-600 dark:text-yellow-300">{xp}</span>
                </div>
              </>
            )}
            ...
            {user ? (
                <div className="relative" ref={menuRef}>
                    ...
                    {isMenuOpen && (
                        <div className="absolute right-0 mt-2 w-60 bg-surface rounded-lg shadow-xl border border-border py-2 animate-scale-in origin-top-right">
                ...
                >
                    {isGuest ? 'Đăng nhập' : 'Đăng nhập / Đăng ký'}
                </button>
            )}
        </div>
      </div>
