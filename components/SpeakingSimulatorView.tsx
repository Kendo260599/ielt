...
const SpeakingSimulatorView: React.FC<SpeakingSimulatorViewProps> = ({ topic, onFinish }) => {
  ...
  const renderContent = () => {
    ...
    return (
      <>
        <div className="bg-gray-100 dark:bg-gray-700 p-6 rounded-lg min-h-[150px] flex flex-col justify-center text-center">
            {isPreparing && (
                <>
                    <p className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-2">Chuẩn bị trong 1 phút</p>
                    <p className="text-5xl font-bold text-indigo-600 dark:text-indigo-400">{timer}</p>
                </>
            )}
            {isSpeaking && currentPart === 2 && (
                <>
                    <p className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-2">Nói trong tối đa 2 phút</p>
                    <p className="text-5xl font-bold text-indigo-600 dark:text-indigo-400">{timer}</p>
                </>
            )}
             {(isSpeaking && currentPart !== 2) && (
                <div className="flex flex-col items-center justify-center">
                    <p className="text-lg font-medium text-gray-700 dark:text-gray-300">Đang lắng nghe...</p>
                    <div className="flex items-center text-indigo-500 mt-4">
                        <MicIcon />
        ...
        <div className="mt-6 flex-grow overflow-y-auto pr-2 space-y-4">
            {transcript.map((item, index) => (
                <div key={index} className={`flex items-start gap-3 ${item.speaker.startsWith('user') ? 'justify-end' : 'justify-start'}`}>
                    {item.speaker === 'model' && <div className="w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center text-white font-bold flex-shrink-0 mt-1">G</div>}
                    <div className={`max-w-[85%] p-3 rounded-2xl ${
                        item.speaker.startsWith('user') ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-100 rounded-bl-none'
  ...
  return (
    <div className="bg-white dark:bg-gray-800 w-full h-[80vh] max-h-[700px] rounded-2xl shadow-lg flex flex-col p-4 sm:p-6 animate-fade-in">
        <div className="flex-shrink-0 flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold">Part {currentPart}</h2>
        ...
        {renderContent()}
         <div className="flex-shrink-0 pt-4 text-center">
             {simulatorState === 'SPEAKING' && currentPart !== 2 && (
                <button onClick={handleNext} className="px-8 py-3 bg-green-600 text-white font-bold rounded-full hover:bg-green-700 transition-colors">
                    Câu tiếp theo
                </button>
             )}
             {currentPart === 3 && questionIndex === questions.part3.length - 1 && (
                 <button onClick={handleNext} className="px-8 py-3 bg-indigo-600 text-white font-bold rounded-full hover:bg-indigo-700 transition-colors">
