...
import { fetchTopics, fetchVocabulary, generateTest, generatePlacementTest, evaluatePlacementTest, fetchSpeakingAnalysis, fetchWordDetails, createTutorChatSession } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const [appState, setAppState] = useState<AppState>(AppState.SHOWING_LOGIN);
  ...
  useEffect(() => {
    ...
    if (user && userData) {
      ...
      // If we are on the login screen (e.g., just logged in), transition to the app
  ...
  const handleRestart = useCallback(() => {
    setAppState(user ? AppState.SHOWING_DASHBOARD : AppState.SHOWING_LOGIN);
  ...
  const handleConfirmStartSimulator = () => {
      ...
      } catch (err)
 {
  ...
  const handleSignOut = useCallback(async () => {
    try {
        ...
        // The useEffect hook will catch user becoming null and reset state to login.
    } catch (error) {
  ...
  const renderContent = () => {
    ...
    if (error) {
      return (
        <div className="text-center p-8 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-xl">
          <h2 className="text-xl font-bold text-red-700 dark:text-red-300">Đã xảy ra lỗi</h2>
    ...
    if (appState === AppState.SHOWING_LOGIN) {
        return <LoginView onSignIn={handleSignIn} onContinueAsGuest={handleContinueAsGuest} isLoading={signInState.isLoading} error={signInState.error} />;
    }
    ...
    switch (appState) {
      ...
      case AppState.IN_LESSON:
        return <LessonView vocabulary={vocabulary} onStartTest={handleStartTest} onBack={handleBackToTopicSelect} onPlayAudio={handlePlayAudio} audioState={audioState} onAskTutor={handleAskTutorForWord} />;
      ...
      case AppState.IN_VOCAB_PRACTICE:
        return <PracticeView vocabulary={vocabulary} onBack={handleBackToTopicSelect} onPlayAudio={handlePlayAudio} audioState={audioState} />;
      case AppState.IN_SRS_REVIEW:
        return <ReviewView vocabulary={reviewWords} onBack={handleBackToDashboard} onPlayAudio={handlePlayAudio} audioState={audioState} onUpdateMastery={handleUpdateMastery} />;
      ...
      case AppState.IN_SPEAKING_SIMULATOR:
        return <SpeakingSimulatorView topic={selectedTopic || 'a topic of your choice'} onFinish={handleFinishSpeakingTest} />;
  ...
  const showHeader = appState !== AppState.SHOWING_LOGIN;

  return (
    <div className="min-h-screen flex flex-col">
      {showHeader && <Header user={user} streak={userData?.streak ?? 0} xp={userData?.totalXp ?? 0} onViewProgress={handleViewProgress} onHomeClick={handleBackToDashboard} showHomeButton={appState !== AppState.SHOWING_DASHBOARD && appState !== AppState.SHOWING_INTRO} onSignOut={handleSignOut} onRequestSignIn={handleRequestSignIn} />}
      <main className="flex-grow flex flex-col container mx-auto p-4 sm:p-6 md:p-8">
